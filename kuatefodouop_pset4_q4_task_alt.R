...
## question 1.4 on Odyssey, for alternative algorithm
...
if (Sys.getenv("SLURM_JOB_ID") != "") { # Divide computation per tasks
  ...
  max_x <- ifelse(data.id == 1, 200, 600)
  chain <- mcmc.mh_Sexplogonly(data, mcmc.niters=N.it, max_x)
  dev.off()
  save("chain", file=paste("./out/", data.name, "_chain_", chain.id, "_alt", ".RData", sep=""))
